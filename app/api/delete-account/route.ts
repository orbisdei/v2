...
export async function POST() {
  ...
  try {
    ...
    // Step 5: Delete site_contributor_notes
    {
      const { error } = await admin
        .from('site_contributor_notes')
        .delete()
        .eq('created_by', uid);
      if (error) throw { step: 5, error };
    }

    // Step 6: Null out created_by on sites
    {
      const { error } = await admin
        .from('sites')
    ...
    // Step 7: Null out created_by on tags
    {
      const { error } = await admin
        .from('tags')
        .update({ created_by: null })
    ...
    // Step 8: Null out updated_by on site_config
    {
      const { error } = await admin
        .from('site_config')
