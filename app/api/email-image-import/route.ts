...
export async function POST(request: NextRequest) {
  ...
  const { sender_email, subject, staging_key, filename } = body as {
    sender_email?: string;
    subject?: string;
    staging_key?: string;
    filename?: string;
  ...
  const missing = ['sender_email', 'subject', 'staging_key', 'filename'].filter(
  ...
  // 5. Fetch raw image from R2 staging
  ...
  // 9. Delete staging object from R2 (best-effort)
  try {
    await r2Client.send(
