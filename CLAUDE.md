...
app/
  ...
  api/
    ...
    revalidate-catalog/route.ts         # Hourly cron (vercel.json): if site_config.catalog_dirty is true, revalidateTag(SITES_TAG/TAGS_TAG) — the catalog-wide bust homepage/search's aggregate caches need — then clears the flag. Skips (no writes) if nothing changed since the last run.
...
- **site_config** — key (text PK), value (jsonb), updated_at, updated_by (uuid → auth.users). Admin-configurable app settings. RLS: public SELECT, admin-only INSERT/UPDATE. Current keys: `homepage_default_levels` (json array of interest levels), `location_tag_high_threshold` (number), `location_tag_low_threshold` (number), `catalog_dirty` (bool — set by lib/revalidate.ts on every scoped site/tag edit, written via the service-role client since RLS restricts writes to admins; cleared by the hourly revalidate-catalog cron).
