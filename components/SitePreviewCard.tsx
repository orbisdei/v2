...
export default function SitePreviewCard({ site, tags, onClose, className = '' }: SitePreviewCardProps) {
  const thumbnail = site.images?.[0]?.url;

  return (
    <div className={`relative ${className}`} style={{ padding: '10px 12px 12px' }}>
      {/* Close button */}
      <button
        onClick={onClose}
      ...
      {/* Top row: optional thumbnail + name/actions */}
      <div className="flex gap-[10px] items-start">
        ...
        <div className="flex-1 min-w-0 pr-6">
          <p
            ...
            style={{ fontSize: 14, fontWeight: 500, lineHeight: 1.3, marginBottom: 6
            }}
          >
            {site.name}
          </p>
          ...
          {/* Visited · Bookmark · Directions
              30px wrappers match the visible circle size exactly, so the layout
              footprint equals the visual size. The 44px touch target overflows
              (overflow: visible) without adding dead space to the layout. */}
      ...
      {/* Tags — horizontal scroll, no More link */}
      {tags.length > 0 && (
        <div
