    ...
    <header className="bg-navy-900 text-white relative z-50">
      <div className="grid grid-cols-3 items-center px-4 md:px-6 h-14">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
          ...
          {user && ['contributor', 'administrator'].includes(user.role) && (
            <Link href="/contribute/new-site" className="hover:text-gold-400 transition-colors">
              Contribute
            </Link>
          )}
        ...
        {/* Empty left column on mobile */}
        <div className="md:hidden" />

        {/* Center: Logo */}
        <Link href="/" className="flex items-center justify-center gap-2">
          <img
            src="/images/orbisdei.png"
            alt=""
            aria-hidden="true"
            className="h-10 w-auto object-contain"
          />
          <span className="font-mont font-bold text-lg tracking-widest uppercase">
            Orbis Dei
          </span>
        </Link>
      ...
      {mobileMenuOpen && (
        <nav className="md:hidden bg-navy-800 border-t border-navy-700 px-4 py-3 flex flex-col gap-3 text-sm">
          <Link href="/" className="py-1.5 hover:text-gold-400" onClick={() => setMobileMenuOpen(false)}>
            Home
          </Link>
          <Link href="/search" className="py-1.5 hover:text-gold-400" onClick={() => setMobileMenuOpen(false)}>
            Search
          </Link>
          <Link href="/about" className="py-1.5 hover:text-gold-400" onClick={() => setMobileMenuOpen(false)}>
            About
          </Link>
          {user && ['contributor', 'administrator'].includes(user.role) && (
            <Link href="/contribute/new-site" className="py-1.5 hover:text-gold-400" onClick={() => setMobileMenuOpen(false)}>
              Contribute
            </Link>
          )}
          {user?.role === 'administrator' && (
            <Link href="/admin" className="py-1.5 hover:text-gold-400" onClick={() => setMobileMenuOpen(false)}>
              Admin
            </Link>
          )}
            ...
            {user ? (
              <div className="flex flex-col gap-2">
                ...
                <form action="/auth/signout" method="post">
                  <button
                    type="submit"
                    className="flex items-center gap-2 text-sm hover:text-gold-400"
                  >
                    <LogOut size={14} />
                    Sign out
                  </button>
